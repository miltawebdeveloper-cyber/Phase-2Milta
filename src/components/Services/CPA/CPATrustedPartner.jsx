...
const TRUST_POINTS = [
  { icon: VerifiedIcon,    title: 'Back-Office Accounting Support',    desc: 'Assisting CPA firms with routine accounting tasks to improve efficiency and reduce turnaround time.' },
  { icon: ReceiptLongIcon, title: 'Tax Preparation Assistance',         desc: 'Supporting peak season demands to ensure timely, accurate submissions for all client filings.' },
  { icon: StorageIcon,     title: 'Data Entry & Reconciliation',        desc: 'Accurate data management and account reconciliations for smooth, error-free financial operations.' },
  { icon: AssignmentIcon,  title: 'Audit Preparation & Documentation',  desc: 'Helping CPA firms prepare essential documents and records for full audit readiness.' },
  { icon: BarChartIcon,    title: 'Financial Analytics & Advisory',     desc: 'Providing actionable insights and data-driven analytics to help firms enhance client service and decision-making.' },
];
    ...
    <Box sx={{ backgroundColor: '#F7FAF7', position: 'relative' }}>
          ...
          <Box sx={{ textAlign: 'center', mb: { xs: 6, md: 8 } }}>
            ...
            <motion.div {...fadeUp(0.1)}>
              <Typography variant="h2" sx={{ fontSize: { xs: '2rem', md: '3rem' }, lineHeight: 1.2, color: '#ffffff', maxWidth: 720, mx: 'auto', letterSpacing: '-0.02em' }}>
                How Miltafs Supports{' '}
                <Box component="span" sx={{ color: alpha(primary, 0.9) }}>CPA Firms in the USA</Box>
              </Typography>
            </motion.div>
            <motion.div {...fadeUp(0.18)}>
              <Typography sx={{ color: alpha('#fff', 0.6), fontSize: '1rem', lineHeight: 1.8, maxWidth: 580, mx: 'auto', mt: 2, fontFamily: '"Outfit", sans-serif' }}>
                Over a decade of trusted expertise and a dedicated team — all working together to help CPA firms deliver exceptional results for their clients.
              </Typography>
            </motion.div>
          </Box>
        </Container>
      ...
      <Container maxWidth={false} sx={{ maxWidth: '1300px', mx: 'auto', px: { xs: 3, md: 4 }, position: 'relative', zIndex: 2 }}>
        <Box sx={{ mt: { xs: -12, md: -22 }, pb: { xs: 8, md: 12 } }}>
          ...
          <Box sx={{ display: 'flex', gap: 2.5, justifyContent: 'center', flexDirection: { xs: 'column', sm: 'row' } }}>
            {TRUST_POINTS.slice(3).map((pt, i) => {
              const Icon = pt.icon;
              return (
                <motion.div key={pt.title} {...fadeUp((i + 3) * 0.1)} style={{ flex: '0 1 calc(33.333% - 10px)', maxWidth: 440 }}>
                  <Box sx={{
                    p: { xs: 3.5, md: 4 }, borderRadius: '22px',
                    bgcolor: 'background.default', border: '1px solid rgba(0,0,0,0.06)',
...
export default CPATrustedPartner;
